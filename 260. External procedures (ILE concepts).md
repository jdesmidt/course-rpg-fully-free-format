...
### Exportable subprocedures

A procedure only becomes visible outside its own module when it is declared with the `export` keyword. Without it, the procedure stays private to its module, not even another module bound into the same service program can call it.

```
   dcl-proc calcTax export;
      dcl-pi calcTax packed(9:2);
         pAmount packed(7:2) const;
      end-pi;

      return roundAmount(pAmount * 0.21);        // a private helper, from the same module
   end-proc;

   dcl-proc roundAmount;
      dcl-pi roundAmount packed(9:2);
         pAmount packed(9:4) const;
      end-pi;

      return %dec(pAmount:9:2);
   end-proc;
```

- `calcTax`, declared `export`, can be called from other modules, and can be listed in binder language (below) to become part of a service program's public signature.
- `roundAmount`, without `export`, stays private to this module: only `calcTax` (or anything else in the same module) can call it, it can be renamed or removed freely without affecting any other module.
- `export` and binder language work at two different levels: a procedure can be `export` and still not be listed in binder language, it is then callable by other modules bound into the same service program, but not by anything outside it.
