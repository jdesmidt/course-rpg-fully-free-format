...
**FREE    
   ...
   *inlr = *on;
```
- Fully free format RPG is recognized by its first line: **FREE must be on the first line, in the first position
...
OPM (Original Program Model) is the old program model, where sources are compiled in 1 step. 

ILE (Integrated Language Environment) is the modern, modular successor to the older OPM. Compilation is done in two steps : first sources are compiled into module objects (*MOD) and  bound into an executable (*PGM or *SRVPGM).

RPG Fully Free Format supports only ILE.