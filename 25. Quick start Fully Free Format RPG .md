...
- Statements can be written on one of multiple lines, but ends with a semi-colon
...
- Before the first record is read the first time trough the cycle, the program resolves any parameters passed to it, does initialization and processes output operations (like printing the header lines).
...
In Fully Free Format RPG, no primary files exist any more, but the RPG logic cycle is still there. Therefor programmers needs to take it into account and set the LR indicator at the end of their program.
...
```
   ...
   snd-msg *ESCAPE 'No records processed';  
```
...
![binding](./images/ile-program-creation.png)
...
Altought this is a two step process, IBM i provides a command to compile a source directly into a program.
